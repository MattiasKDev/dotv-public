...
function showUI() {
  let box = document.getElementById('statsplus-ui');

  if (!box) {
    box = Object.assign(document.createElement('div'), { id: 'statsplus-ui' });
  ...
  subtitle.textContent = 'by Infinity';
  ...
  const statPointsPerLevel = globalStats.lvl > 0 ? (globalStats.sp / globalStats.lvl).toFixed(2) : 0;

  const stat1 = document.createElement('div');
  applyStyles(stat1, styles.statInfoItem);
  stat1.textContent = `Stat Points/Level = ${statPointsPerLevel}`;

  const stat2 = document.createElement('div');
  applyStyles(stat2, styles.statInfoItem);
  stat2.textContent = `Stats/Level = ${globalStats.lvl > 0 ? (statSum / globalStats.lvl).toFixed(2) : 0}`;
